...
**Applied and complete.** The board label is **`CDPHP`**, byte-identical on both
boards — verified against `settings_str`, so the Subscription → Order copy matches
and does not blank (§2).

| Step | State |
|---|---|
| Subscription board `color_mm254qxj` | **done** — index **107**, colour `navy`, position 24 |
| Order board `color_mm18jhq5` | **done** — index **153**, colour `sky`, position 27 |
| `subscription/workflow.ts` `PRIMARY_INSURANCE_OPTIONS` | **done** — `{ index: 107, label: "CDPHP" }`, index read back from the live column, not assumed |
| Cardinal `insurancemap.js` `ROUTING` | **done** — `'CDPHP': split('CDPHP', 'Managed Medicaid', 'Managed Medicaid')` |

It was first created as
`CDPHP (capital district physicans' healthcare network)` and renamed to `CDPHP`
in the Monday UI on the same day. The UI rename **preserved the index, the colour,
the curated position and `color_mapping` on both columns** — read back and
confirmed — which is exactly what §9.1 predicted the API route would have put at
risk. The old spelling exists nowhere now, so the transitional second `ROUTING`
key that covered it has been removed; the table mirrors the boards again.
...
**If CDPHP is ever renamed again, rename both boards together or not at all** —
renaming one blanks the hop (§2) — and **do it in the Monday UI, not the API**
(§9.1). Two things in code follow a rename: the `label` string in
`subscription/workflow.ts` (display-only — the index is the binding), and
`CDPHP_BOARD_LABEL` in `cardinal-api/test/transform.test.js`, which pins the board
spelling so a rename fails a test instead of failing orders.
...
**Scope is deliberately partial** (confirmed by Josh): CDPHP is on the
